...
- [x] **Install Zustand + localforage** — add `zustand`, `zustand/middleware`, and `localforage` deps; set up example persisted store.
...
_Implement search capabilities for products and vendors with offline map support for faster results and reduced data usage._
...
- [ ] **Implement PMTiles regional downloads** — allow users to download vector tile regions based on managed countries data.
...
- [ ] **Optimize bandwidth usage** — prioritize offline tiles for search result display and fall back to online when needed.
